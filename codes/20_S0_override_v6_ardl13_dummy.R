...
# ── Logging ───────────────────────────────────────────────────────────
out_dir <- here::here("output", "CriticalReplication", "S0_manualOverride","ardl13_dummy")
...
#Lag Selection Order 
ardl_S0v1_bic <- auto_ardl(lnY ~ lnK | d1956 + d1974 +d1980, data = df_ts, max_order = c(5,5), selection = "BIC")
ardl_S0v1_aic <- auto_ardl(lnY ~ lnK | d1956 + d1974 +d1980, data = df_ts, max_order = c(5,5), selection = "AIC")
...
readr::write_csv(AIC_BIC_top5, file.path(out_dir, "AIC_BIC_top5_dummy.csv"))

#Run ARDL
ardl_s0v1 <- ardl(lnY ~ lnK | d1956 + d1974 +d1980, data = df_ts, order = c(1,3))
summary(ardl_s0v1)
...
ardl_S0v1_bic <- auto_ardl(lnY ~ lnK + trend(lnY) | d1956 + d1974 +d1980, data = df_ts, max_order = c(5,5), selection = "BIC")
ardl_S0v1_aic <- auto_ardl(lnY ~ lnK + trend(lnY) | d1956 + d1974 +d1980, data = df_ts, max_order = c(5,5), selection = "AIC")
AIC <- ardl_S0v1_aic$top_orders
BIC <- ardl_S0v1_bic$top_orders
AIC_BIC_top5 <- AIC |> dplyr::left_join(BIC, by = c("lnY","lnK"))
AIC_BIC_top5 <- round(AIC_BIC_top5[1:5,],2)

readr::write_csv(AIC_BIC_top5, file.path(out_dir, "AIC_BIC_top5_trend_dummy.csv"))

#Trend holds AIC (p,1) = (1,3)
ardl_s0v1t <- ardl(lnY ~ lnK + trend(lnY) | d1956 + d1974 +d1980, data = df_ts, order = c(1,3))
summary(ardl_s0v1t)
...
#Case 1 check 
ardl_S0v1_bic <- auto_ardl(lnY ~ lnK -1 | d1956 + d1974 +d1980, data = df_ts, max_order = c(5,5), selection = "BIC")
ardl_S0v1_aic <- auto_ardl(lnY ~ lnK -1 | d1956 + d1974 +d1980, data = df_ts, max_order = c(5,5), selection = "AIC")
AIC <- ardl_S0v1_aic$top_orders
BIC <- ardl_S0v1_bic$top_orders
AIC_BIC_top5 <- AIC |> dplyr::left_join(BIC, by = c("lnY","lnK"))
AIC_BIC_top5 <- round(AIC_BIC_top5[1:5,],2)
readr::write_csv(AIC_BIC_top5, file.path(out_dir, "AIC_BIC_top5_noconst_dummy.csv"))


#NOTE ON LAG SELECTION: WE KEEP IN 1,3 to hold consistency, it remains in top 5 within top orders in BIC and AIC 
ardl_s0v1nc <- ardl(lnY ~ lnK -1 | d1956 + d1974 +d1980, data = df_ts, order = c(1,3))
summary(ardl_s0v1nc)
...
readr::write_csv(PSS2001_CI_bounds_test, file.path(out_dir, "PSS2001_CI_bounds_test_ardl13dummy.csv"))
...
ardl13_dummy <- here::here("output", "CriticalReplication", 
                          "S0_manualOverride", "ardl13_dummy")

aic_bic_table <- purrr::map_dfr(
  list(
    list(file = "AIC_BIC_top5_noconst_dummy.csv", model_class = "no_const",    cases = "Case 1"),
    list(file = "AIC_BIC_top5_dummy.csv",          model_class = "const",       cases = "Cases 2–3"),
    list(file = "AIC_BIC_top5_trend_dummy.csv",   model_class = "const_trend", cases = "Cases 4–5")
  ),
  function(entry) {
    readr::read_csv(file.path(ardl13_dummy, entry$file), show_col_types = FALSE) |>
...
readr::write_csv(aic_bic_table, 
                 file.path(ardl13_dummy, "AIC_BIC_consolidated_dummy.csv"))
...
aic_bic_tex <- aic_bic_table |>
  ...
  knitr::kable(
    ...
    booktabs  = TRUE,
    linesep   = "",
    caption   = "Top-5 ARDL lag orders by model class, with step dummies (d1956, d1974, d1980). Dependent variable: $\\ln Y$; regressor: $\\ln K$.",
    label     = "tab:aic_bic_dummy",
    align     = c("l", "l", "c", "c", "r", "r"),
    na        = "---"
  ) |>
...
writeLines(
  aic_bic_tex,
  file.path(ardl13_dummy, "AIC_BIC_consolidated_dummy.tex")
)

# ── LaTeX export — PSS2001 bounds test table ──────────────────────────

PSS2001_CI_bounds_test |> as.data.frame() |>
  ...
  knitr::kable(
    ...
    caption  = "PSS (2001) bounds test for cointegration, Cases I--V. ARDL(2,4) with step dummies d1956, d1974, d1980. Exact $p$-values conditioned on $T=65$.",
    label    = "tab:pss2001_bounds",
    align    = c("l", "r", "r", "r", "r"),
    na       = "---",
    escape   = FALSE
  ) |>
  ...
  kableExtra::footnote(
    general = "t-bounds test is structurally undefined for Cases II and IV (PSS 2001). $p$-values computed via simulation with $R = 40{,}000$ draws using \\\\texttt{exact = TRUE}.",
    general_title = "Note:",
    footnote_as_chunk = TRUE,
    escape = FALSE
  ) |>
  writeLines(file.path(ardl13_dummy, "PSS2001_CI_bounds_test.tex"))
